...
Migrated from utils/data_viz.py - functions generate Plotly figures or matplotlib figures.
"""
import pandas as pd
import plotly as pt
...
color_discrete_map_presence = pd.Series({
    'Started': '#0068c9',
    'Sub in': '#83c9ff',
    'Not played': '#ff2b2b',
})
...
def plot_pitch(field_dimen=(120, 80), 
    ...
    Args:
        ...
        showgrid (bool, optional): Whether to show grid. Defaults to False.
        showaxis (bool, optional): Whether to show axis. Defaults to False.
    ...
    fig.update_layout(
        ...
        height=height,
        xaxis=dict(range=xrange, showgrid=showgrid, zeroline=False, visible=showaxis),
        yaxis=dict(range=yrange, showgrid=showgrid, zeroline=False, visible=showaxis)
    )
...
def plot_pitch_areas(n_rows=3, n_cols=3, field_dimen=(120, 80), fig=None, show_numbers=True):
    """
    Plot pitch areas with grid lines and zone numbers.
    
    Args:
        n_rows: Number of rows in the grid
        n_cols: Number of columns in the grid
        field_dimen: Field dimensions (length, width)
        fig: Optional existing Plotly figure to add areas to
        show_numbers: Whether to show zone numbers
    Returns:
        go.Figure: Plotly figure with pitch areas
        dict: Dictionary mapping zone numbers to (x, y) center coordinates
    """
    length, width = field_dimen
    v_step = width / n_rows
    h_step = length / n_cols

    if not fig: 
        fig, _ = plot_pitch(field_dimen=field_dimen)

    # Plot lines
    for l in range(1, n_rows):
        fig.add_shape(type="line", x0=0, y0=(l * v_step), x1=length, y1=(l * v_step),
                      line=dict(color='gray', width=3), layer="below")

    for l in range(1, n_cols):
        fig.add_shape(type="line", x0=(l * h_step), y0=0, x1=(l * h_step), y1=width,
                      line=dict(color='gray', width=3), layer="below")

    
    # Plot dots
    # Number zones row by row (left to right), matching frontend PitchSelector
    # Note: In Plotly, y=0 is at bottom, so we iterate rows in reverse to match HTML canvas (y=0 at top)
    zone_n = 0
    zone_dict = {}
    for v in range(n_rows - 1, -1, -1):  # Iterate rows from top to bottom (reverse order)
        for h in range(n_cols):  # Then columns left to right
            x = h_step / 2 + h * h_step
            y = v_step / 2 + v * v_step
            zone_dict[zone_n] = (x, y)
            if show_numbers:
    ...
    return fig, zone_dict


def create_grid(cell_centers, hot_dict, rows, columns, field_dimen=(120, 80), fig=None):
    """
    Create a heatmap grid showing event percentages.
    
    Args:
        cell_centers: Dictionary mapping zone numbers to (x, y) center coordinates
        hot_dict: Dictionary mapping zone numbers to event counts
        rows: Number of rows in the grid
        columns: Number of columns in the grid
        field_dimen: Field dimensions (length, width)
        fig: Optional existing Plotly figure to add grid to
        
    Returns:
        go.Figure: Plotly figure with heatmap grid
    """
    # Calculate cell width and height based on grid dimensions and number of rows/columns
    cell_length = field_dimen[0] / columns
    cell_width = field_dimen[1] / rows
    
    # Create a new figure without pitch drawing
    if not fig:
        fig = go.Figure()
        # Set axis ranges to match field dimensions
        padding = 2
        fig.update_layout(
            xaxis=dict(range=[-padding, field_dimen[0] + padding], showgrid=False, zeroline=False, visible=False),
            yaxis=dict(range=[-padding, field_dimen[1] + padding], showgrid=False, zeroline=False, visible=False),
            plot_bgcolor='white',
            width=800,
            height=600,
        )
    ...
    counts = list(hot_dict.values())
    total_count = sum(counts) if counts else 1
    
    # Calculate font size based on number of zones (more zones = smaller font)
    total_zones = rows * columns
    if total_zones <= 9:  # 3x3 or smaller
        font_size = 20
    elif total_zones <= 16:  # 4x4
        font_size = 16
    elif total_zones <= 25:  # 5x5
        font_size = 14
    else:  # 6x6 or larger
        font_size = 12
    
    # Add cell centers and rectangles
    # Iterate zones in order (0, 1, 2, ...) to match frontend numbering (row by row)
    for key in sorted(cell_centers.keys(), key=int):
        x_center, y_center = cell_centers[key] 
        count = hot_dict.get(key, 0)
        per_count = round(count / total_count * 100, 2) if total_count > 0 else 0
        
        # Format percentage text: hide if very small, format based on value
        if per_count < 0.1:
            text = ''  # Hide very small percentages to avoid clutter
        elif per_count >= 1:
            # For >= 1%, show with 1 decimal or as integer if whole number
            text = f'{per_count:.1f}%' if per_count % 1 != 0 else f'{int(per_count)}%'
        else:
            # For < 1%, show with 1 decimal
            text = f'{per_count:.1f}%'
        
        # Color based on percentage (0-100%): from white (0%) to red (100%)
        # Normalize percentage to 0-1 range for opacity
        opacity = per_count / 100.0
        color = f'rgba(255, 0, 0, {opacity})'  # Red color with opacity based on percentage
        ...
        if text:  # Only add text if not empty
            fig.add_trace(go.Scatter(
                ...
                name='',
                text=text,
                textfont=dict(color='white' if opacity > 0.5 else 'black', size=font_size),
                textposition='middle center',
                showlegend=False
            ))
        
        left = (x_center - cell_length / 2)
        bottom = (y_center - cell_width / 2)
        right = (x_center + cell_length / 2)
        top = (y_center + cell_width / 2)
        
        # Add rectangle with colored background
        fig.add_shape(
            type='rect',
            x0=left,
            y0=bottom,
...
def plot_football_field_with_positions(player_positions=None, width=800, height=600):
    """
    Plot football field with player positions.
    
    Args:
        player_positions: DataFrame with player position data
        width: Plot width
        height: Plot height
        
    Returns:
        go.Figure: Plotly figure with player positions
    """
...
def create_radar_chart(traces, labels, title='', fill=False, 
    ...
    for trace_name, trace in traces.items():
        ...
        percentiles = trace['percentiles'] + [trace['percentiles'][0] - 0.1]
...
def create_bar_chart(labels, 
                     ...
                     orientation='h', title='', resize_by='width', y_inverted=False,
