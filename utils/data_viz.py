...
def plot_pitch_areas(n_rows=3, n_cols=3, field_dimen=(120,80), fig = None):
...
def create_grid(cell_centers, hot_dict, rows, columns, field_dimen=(120,80), fig = None):
    ...
    if not fig:
        fig, _ = plot_pitch(field_dimen)

    # Normalize the count values for color scaling
    ...
    for key, (x_center, y_center) in cell_centers.items(): 
        count = hot_dict.get(key, 0)
        normalized_count = (count - min_count) / (max_count - min_count) if max_count != min_count else 0.0
        per_count = round(count/sum(counts)*100,2) if sum(counts)>0 else 0
        color = f'rgba(255, 0, 0, {normalized_count})'  # Red color with varying opacity
        ...
        fig.add_shape(
            ...
            fillcolor=color,  # Transparent fill
            layer='below'
        )
